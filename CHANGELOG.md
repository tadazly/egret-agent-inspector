...
- 拖出去才生效的控件：自己只挂 `touchBegin`、按下时才挂松手回调的控件（战斗里的换宠卡要拖出卡片上沿才换上场），动作表标「按住上滑」这类状态；按编号点它时 `egret_act` 自动按住滑出去，方向从松手回调里认（`globalToLocal` 后 `y < 0` 即上滑）。认不出方向时标「按住拖出去」，新增 `{"op":"swipe","dir":…}`。
