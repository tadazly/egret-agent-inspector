...
环境变量：
  ...
  EGRET_OCR_BACKEND       本地 OCR 引擎：auto（默认，Windows 用系统 Windows ML 跑 PP-OCRv4 识别模型，
                          macOS 用 Vision）、native（只用 Windows.Media.Ocr / Vision）、ppocr（只用识别模型，起不来就报错）
  EGRET_OCR_MODEL         识别模型 onnx 的路径；默认首次使用时下载到 ~/.egret-agent-inspector/ocr
  EGRET_OCR_PREWARM       置 0 时不在启动时预热 OCR 常驻进程
"""
...
OCR_WINDOWS_ML_SOURCE = os.path.join(PLUGIN_ROOT, "scripts", "ocr_windows_ml.ps1")
# Windows 走系统自带的 Windows ML 跑 PaddleOCR PP-OCRv4 mobile 识别模型：首次使用时下载到用户目录，
# 不进仓库；下载不了或 Windows ML 起不来就退回 Windows.Media.Ocr。
OCR_MODEL_NAME = "ch_PP-OCRv4_rec_mobile.onnx"
OCR_MODEL_SIZE = 10857958
OCR_MODEL_SHA256 = "48fc40f24f6d2a207a2b1091d3437eb3cc3eb6b676dc3ef9c37384005483683b"
# 依次尝试：RapidAI 在 ModelScope 上按版本 tag 固定的公开地址（PaddleOCR 官方模型转成 ONNX，Apache-2.0），
# 再试本仓库 GitHub Release 的镜像附件；都不通时用 EGRET_OCR_MODEL 指定手动下载的文件
OCR_MODEL_URLS = (
    "https://www.modelscope.cn/models/RapidAI/RapidOCR/resolve/v3.9.2/onnx/PP-OCRv4/rec/ch_PP-OCRv4_rec_mobile.onnx",
    "https://github.com/tadazly/egret-agent-inspector/releases/download/ocr-model-v1/ch_PP-OCRv4_rec_mobile.onnx",
)
OCR_BACKEND = (os.environ.get("EGRET_OCR_BACKEND") or "auto").strip().lower()  # auto | native | ppocr
# 动作表里「ocr 补了 N 个标签（ppocr 120ms）」用的短名，方便验收记录里分辨用的是哪个引擎
OCR_BACKEND_SHORT = {"windows-ml-ppocr": "ppocr", "windows-media-ocr": "winocr",
                     "macos-vision-accurate-warm": "vision", "macos-vision": "vision-1shot"}
_OCR_WORKER = None
_OCR_WORKER_BACKEND = None
...
def ocr_model_path():
    custom = os.environ.get("EGRET_OCR_MODEL")
    if custom:
        return os.path.expanduser(custom)
    return os.path.join(os.path.expanduser("~"), ".egret-agent-inspector", "ocr", OCR_MODEL_NAME)
...
def ensure_ocr_model():
    """识别模型不在本机就下载（约 10MB），校验 SHA-256 后才启用；返回模型路径，失败抛异常。"""
    path = ocr_model_path()
    if os.path.isfile(path):
        if os.environ.get("EGRET_OCR_MODEL") or (os.path.getsize(path) == OCR_MODEL_SIZE and _sha256_of(path) == OCR_MODEL_SHA256):
            return path
        os.unlink(path)
    if os.environ.get("EGRET_OCR_MODEL"):
        raise RuntimeError("EGRET_OCR_MODEL 指向的模型不存在：" + path)
    import urllib.request
    os.makedirs(os.path.dirname(path), exist_ok=True)
    errors = []
    for url in OCR_MODEL_URLS:
        part = path + ".part"
        try:
            log("downloading OCR model (%.1f MB) from %s" % (OCR_MODEL_SIZE / 1e6, url))
            with urllib.request.urlopen(url, timeout=60) as resp, open(part, "wb") as out:
                shutil.copyfileobj(resp, out, 1 << 20)
            if _sha256_of(part) != OCR_MODEL_SHA256:
                raise RuntimeError("下载的模型校验失败")
            os.replace(part, path)
            log("OCR model saved to " + path)
            return path
            ...
            try:
                ...
                pass
    raise RuntimeError("OCR 模型下载失败（可手动下载后用 EGRET_OCR_MODEL 指定路径）：" + "；".join(errors))
...
def windows_powershell():
    ...
    if not powershell:
        raise RuntimeError("Windows OCR 需要 Windows PowerShell")
    return powershell


def _start_worker(command, backend, env=None):
    """拉起常驻 OCR 进程：第一行 {"ready":true} 后才算就绪。stderr 不接管道，免得写多了把进程堵死。"""
    global _OCR_WORKER, _OCR_WORKER_BACKEND
    ...
    if not ready:
        worker.kill()
        raise RuntimeError("%s worker 未就绪：%s" % (backend, (line or "").strip()[:200] or "进程退出"))
    _OCR_WORKER = worker
    _OCR_WORKER_BACKEND = backend
    _OCR_WORKER_READY.set()


def _prewarm_ocr():
    global _OCR_WORKER_ERROR
    try:
        if sys.platform == "darwin":
            binary, _ = macos_ocr_binary()
            _start_worker([binary, "--daemon"], "macos-vision-accurate-warm")
        elif sys.platform == "win32" and OCR_BACKEND != "native":
            model = ensure_ocr_model()
            _start_worker([windows_powershell(), "-NoProfile", "-NonInteractive", "-ExecutionPolicy", "Bypass",
                           "-File", OCR_WINDOWS_ML_SOURCE, "-Model", model, "-Daemon"], "windows-ml-ppocr")
    except Exception as e:  # noqa: BLE001
        _OCR_WORKER_ERROR = str(e)
        log("OCR worker unavailable, falling back to the native engine: " + _OCR_WORKER_ERROR)


def start_ocr_prewarm():
    if sys.platform in ("darwin", "win32") and os.environ.get("EGRET_OCR_PREWARM", "1") != "0" and not _OCR_WORKER_READY.is_set():
        threading.Thread(target=_prewarm_ocr, name="egret-ocr-prewarm", daemon=True).start()
...
def tidy_ocr_text(text):
    """汉字之间不该有空格：Windows.Media.Ocr 把每个汉字当一个词（"福 利 商 城"）。"""
...
def run_fast_ocr(image_data, candidates, scale):
    ...
    优先用常驻进程（macOS Vision accurate / Windows ML + PP-OCRv4）；没起来或起不来就一次性跑
    平台自带引擎（macOS Vision fast / Windows.Media.Ocr）。
    """
    compile_ms = 0
    use_worker = OCR_BACKEND != "native" and _OCR_WORKER_READY.is_set() and _OCR_WORKER and _OCR_WORKER.poll() is None
    if use_worker:
        backend = _OCR_WORKER_BACKEND
        command = None
        ...
        if OCR_BACKEND == "ppocr" and _OCR_WORKER_ERROR:
            raise RuntimeError(_OCR_WORKER_ERROR)
        if not os.path.isfile(OCR_WINDOWS_SOURCE):
    ...
    try:
        ...
        if use_worker:
            with _OCR_WORKER_LOCK:
                ...
                line = _OCR_WORKER.stdout.readline()
            if not line:
                raise RuntimeError("OCR worker 已退出")
            decoded = json.loads(line)
            if decoded.get("error"):
                raise RuntimeError(decoded["error"])
        else:
            # 新编译的 macOS helper 第一次启动（系统校验 + Vision 加载模型）可能要十几秒；Windows 的一次性调用不到 1 秒
            completed = subprocess.run(command + [spec_path], capture_output=True, text=True,
                                       encoding="utf-8", errors="replace", timeout=30 if sys.platform == "darwin" else 8)
            if completed.returncode:
                raise RuntimeError(completed.stderr.strip() or "本地 OCR 执行失败")
            decoded = json.loads(completed.stdout)
        texts, confidences = {}, {}
        for match in decoded.get("matches", []):
            text = tidy_ocr_text(match.get("text"))
...
def render_action_table(table):
    ...
    if newbie.get("done"):
        ...
        if table.get("scope") == "stage" and bag:
            ...
            line += "；剩下的普通界面和接连弹的礼包一次关回地图界面：[{\"op\":\"close\",\"all\":true}]"
        lines.append(line)
    elif guiding:
    ...
    for rec in table.get("executed") or []:
        ...
        if rec.get("op") == "close" and isinstance(rec.get("result"), dict) and result.get("all"):
            layers = [str(p).split("#")[-1].split(".")[-1] for p in result.get("layers") or []]
            line += " → 关掉 %d 层%s" % (len(layers), "（%s）" % "、".join(layers) if layers else "")
            if result.get("stopped") == "hub":
                line += "，已回到地图界面"
            else:
                line += "，停在 %s（%s）：%s" % (result.get("panel") or "", result.get("stopped"), result.get("note") or "")
        elif rec.get("op") == "close" and isinstance(rec.get("result"), dict):
            if result.get("ok") and result.get("via") == "gone":
                line += " → %s 自己退场了" % (result.get("panel") or "")
            elif result.get("ok"):
                via = {"outer": "外层模块的关闭/返回键", "shell": "下层外壳的返回键",
                       "late": "关闭键（退场慢，多等了一下）", "corner": "左上角的返回箭头"}.get(result.get("via"), result.get("via"))
    ...
    if ocr.get("filled"):
        # 全部命中缓存时没有耗时
        engine = OCR_BACKEND_SHORT.get(ocr.get("backend"), "")
        took = "（%s%sms）" % (engine + " " if engine else "", ocr["elapsedMs"]) if ocr.get("elapsedMs") is not None else "（缓存）"
...
TOOLS = {
    ...
    "egret_act": (
        ...
        "{\"op\":\"close\",\"all\":true} 一层层关回地图界面（接连弹出的礼包也等着关掉）；"
    ...
    "egret_notes": (
        ...
        {"type": "object", "properties": {
            ...
            "kind": {"type": "string", "description": "按类型过滤：route 真实 UI 路线、checkpoint 稳定检查点、shortcut 调试直达、recovery 恢复、entry、locator、pitfall、bug、timing、fact、suggestion、account 测试账号（只有指定 kind=account 才返回）"},
            "entries": {"type": "array", "items": {"type": "object"},
                        "description": "add 用：[{kind,key,summary,detail,start,steps,expect,environment,lastVerifiedAt}]；route 的 steps 只记录稳定 UI 定位，不保存 hash；"
                                           "kind=account 时 key 是账号，写 state（一句话现状）、clean（还干净吗）、usedBy（用于什么测试），按字段合并"},
    ...
    "egret_reload_extension": (
        ...
        "连着好几个浏览器时必须带 tabId，只重载持有这个标签页的浏览器；页面卡住、请求超时不要用它。",
        {"type": "object", "properties": {
            "tabId": {"type": "integer", "description": "要重载的是持有这个标签页的浏览器"}}},
...
# kind=account：测试账号登记。登录和新手进度由 server 自动写，state / clean / usedBy 由用完的 agent 补一句
ACCOUNT_FIELDS = ("newbie", "clean", "state", "usedBy", "createdAt", "lastUsedAt", "line")


def account_summary(item):
    """账号一行摘要，搜索「干净 新手已走完」就能挑号。"""
    newbie = item.get("newbie") or {}
    parts = ["新手已走完" if newbie.get("done") else "新手 %s/%s" % (newbie.get("step"), newbie.get("total"))
             if newbie.get("step") is not None else "新手进度未知"]
    parts.append("干净" if item.get("clean") else "用过")
    for field, label in (("state", ""), ("usedBy", "用于：")):
        if item.get(field):
            parts.append(label + str(item[field]))
    return " · ".join(parts)
...
def upsert_account(items, account, fields, now):
    """同一个号只留一条，字段合并：自动登记的新手进度不会被 agent 补的一句现状冲掉。"""
    old = next((it for it in items if it.get("kind") == "account" and it.get("key") == account), None)
    item = dict(old or {"kind": "account", "key": account}, **fields)
    item["updated"] = now
    item["summary"] = account_summary(item)
    return [it for it in items if it is not old] + [item]
...
class Bridge:
    def __init__(self):
        ...
        # 在哪个浏览器里见过的标签页：它那边扩展掉线重连期间，请求别落到别的浏览器上报「No tab with id」
        self.seen_tabs = set()
    ...
    async def route(self, method, params, timeout):
        ...
        # 见过这个标签页、它的浏览器却不在了：扩展掉线后半分钟内会重连（远程桌面断开时常见），等它回来
        if owner is None and tab_id in self.seen_tabs:
            deadline = max(deadline, time.time() + 35)
        while owner is None and time.time() < deadline and (any(c.tabs for c in self.live()) or tab_id in self.seen_tabs):
            self.changed.clear()
            try:
...
class McpServer:
    def __init__(self, bridge):
        ...
        # tabId -> 这个标签页用 splan_call login 登进去的账号；以及每个号上次登记的新手进度
        self.tab_accounts = {}
        self.account_newbie = {}
    ...
    async def call_tool(self, name, args):
        ...
        try:
            ...
            if name == "splan_call" and args.get("action") == "login" and isinstance(res, dict) and res.get("ok") and res.get("account"):
                await self.notes_scope()
                tab = res.get("tabId", args.get("tabId"))
                self.tab_accounts[tab] = res["account"]
                now = int(time.time())
                fields = {"lastUsedAt": now, "line": res.get("onlineId")}
                if args.get("newAccount"):
                    fields.update(createdAt=now, clean=True)
                self.note_account(res["account"], **{k: v for k, v in fields.items() if v is not None})
            ...
            text = str(e)
            # 验收里 agent 看到这句就去 list_tabs，换了别的会话的 Edge、甚至用户自己的 Chrome 接着点
            if text.startswith("No tab with id") and args.get("tabId") is not None:
                text = ("标签页 %s 暂时找不到：持有它的浏览器扩展掉线了，一般半分钟内重连。过一会儿用同一个 tabId 重试；"
                        "不要换用 egret_list_tabs 里别的标签页，那些是别人的。（%s）") % (args["tabId"], text)
    ...
    def note_account(self, account, **fields):
        """账号登记：login 成功时记下号和线路，新手进度变了再写；不知道是哪个号就不记。"""
        if not account or not getattr(self, "scope", None):
            return
        newbie = fields.get("newbie")
        if newbie is not None:
            newbie = {k: newbie.get(k) for k in ("step", "total", "done") if newbie.get(k) is not None}
            if self.account_newbie.get(account) == newbie:
                return
            self.account_newbie[account] = fields["newbie"] = newbie
        try:
            path = notes_file(self.scope)
            write_notes(path, upsert_account(read_notes(path), account, fields, int(time.time())))
        except OSError as e:
            log("account note failed: %s" % e)
    ...
    def note_newbie(self, args, table):
        ...
        if not newbie:
            return
        key = table.get("tabId", args.get("tabId"))
        self.note_account(newbie.get("account") or getattr(self, "tab_accounts", {}).get(key), newbie=newbie)
    ...
    def note_route(self, args, table):
        ...
        for k, (step, rec) in enumerate(zip(sent, recs)):
            ...
            log.append({
                ...
                # 落回了地图界面（没有模态面板，整个舞台进表）：接下来去哪是新的决定
        ...
        for j in range(start - 1, -1, -1):
            ...
            return
        # 回到地图界面就是一段路线的终点：登录后关完同一串弹窗，上次接着去打了 PVE，这次未必（探索验收里就被推荐了上个任务的路线）
    ...
    async def invoke(self, name, args):
        ...
        try:
            ...
            if bridge_method == "reloadExtension":
                # 扩展只认点名了自己标签页的重载，或只连着这一个浏览器时的重载（见 bridge.js）
                params = {"sole": True} if len(self.bridge.live()) < 2 else {}
                if args.get("tabId") is not None:
                    params["tabId"] = args["tabId"]
                res = await self.bridge.request("reloadExtension", params, timeout)
            ...
            if bridge_method == "page":
                ...
                if page_method == "getTree":
                    ...
                    # 没指定时交给页面：面板 30 行，整个舞台进表（地图界面 HUD + 地图）时给到 60 行
    ...
    async def notes(self, args):
        ...
        if action in ("search", "list"):
            ...
            scored = []
            for item in items:
                # 账号登记只在点名要号时出来，别混进找经验的结果里
                if (kind or "") != item.get("kind") and (kind or item.get("kind") == "account"):
                    continue
            ...
            return {"scope": scope, "total": len(hits), "notes": [{k: v for k, v in it.items() if k in NOTE_FIELDS + ACCOUNT_FIELDS}
        ...
        if action == "add":
            ...
            for e in entries:
                if e.get("kind") == "account":
                    if not e.get("key"):
                        raise ValueError("account 笔记的 key 是账号名")
                    items = upsert_account(items, e["key"], {f: e[f] for f in ACCOUNT_FIELDS if e.get(f) is not None}, now)
                    continue
