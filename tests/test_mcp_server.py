...
WINDOWS_OCR_ML = SCRIPTS / "ocr_windows_ml.ps1"
...
class BrowserExtensionTest(unittest.TestCase):
    ...
    def test_windows_ml_ocr_source_is_windows_powershell_compatible(self):
        source = WINDOWS_OCR_ML.read_bytes()
        self.assertTrue(source.startswith(b"\xef\xbb\xbf") or source.isascii())
        if sys.platform == "win32" and shutil.which("powershell"):
            # 模型文件不存在时要干净地报错退出（而不是语法错误或挂住）
            result = subprocess.run(
                ["powershell", "-NoProfile", "-NonInteractive", "-ExecutionPolicy", "Bypass",
                 "-File", str(WINDOWS_OCR_ML), "-Model", str(ROOT / "missing-model.onnx"), "-Daemon"],
                capture_output=True, text=True, encoding="utf-8", errors="replace", timeout=60)
            self.assertNotIn("ParserError", result.stderr)
            self.assertNotEqual(result.returncode, 0)
            self.assertIn("missing-model.onnx", result.stderr)
...
class FakeExtension:
    ...
    def answer(self, msg):
        ...
        if msg["method"] == "reloadExtension":
            self.reloads = getattr(self, "reloads", []) + [params]
...
class AccountNotesTest(unittest.IsolatedAsyncioTestCase):
    async def test_login_and_newbie_progress_are_logged_and_agent_notes_merge(self):
        """测试账号登记：login 和新手进度自动写，agent 补的一句现状按字段合并，按「干净 新手已走完」挑得到。"""
        ...
        with tempfile.TemporaryDirectory() as tmp:
            old_root = server.NOTES_ROOT
            server.NOTES_ROOT = tmp
            try:
                mcp = server.McpServer.__new__(server.McpServer)
                mcp.scope, mcp.tab_accounts, mcp.account_newbie, mcp.newbie_seen = "splan.test", {}, {}, set()

                async def fake_login(name, args):
                    return {"ok": True, "account": "agent1", "onlineId": 10105, "tabId": 7}
                mcp.invoke = fake_login
                await mcp.call_tool("splan_call", {"action": "login", "newAccount": True, "tabId": 7})
                mcp.note_newbie({"tabId": 7}, {"tabId": 7, "newbie": {"step": 3, "total": 24, "guiding": True}})
                # 别的进程登的号：页面报上来的 account 也记
                mcp.note_newbie({"tabId": 9}, {"tabId": 9, "newbie": {"step": 24, "total": 24, "done": True, "account": "agent2"}})
                await mcp.notes({"action": "add", "entries": [{"kind": "account", "key": "agent1", "clean": True}]})
                await mcp.notes({"action": "add", "entries": [
                    {"kind": "account", "key": "agent2", "state": "主宠 Lv21", "clean": False, "usedBy": "加经验"}]})
                listed = await mcp.notes({"action": "list", "kind": "account"})
                by_key = {n["key"]: n for n in listed["notes"]}
                self.assertEqual(by_key["agent1"]["summary"], "新手 3/24 · 干净")
                self.assertEqual(by_key["agent1"]["line"], 10105)
                self.assertEqual(by_key["agent2"]["summary"], "新手已走完 · 用过 · 主宠 Lv21 · 用于：加经验")
                self.assertEqual(by_key["agent2"]["newbie"], {"step": 24, "total": 24, "done": True})
                clean = await mcp.notes({"action": "search", "kind": "account", "q": "干净"})
                self.assertEqual([n["key"] for n in clean["notes"]], ["agent1"])
                await mcp.notes({"action": "add", "entries": [{"kind": "fact", "key": "新手选宠", "summary": "新手选宠点确认一次"}]})
                plain = await mcp.notes({"action": "search", "q": "新手"})
                self.assertEqual([n["key"] for n in plain["notes"]], ["新手选宠"])
            finally:
                server.NOTES_ROOT = old_root
...
class OcrLabelTest(unittest.TestCase):
    ...
    def test_tidy_ocr_text_joins_cjk_characters(self):
        sys.path.insert(0, str(SERVER.parent))
        try:
            import egret_agent_inspector_mcp as server
        finally:
            sys.path.pop(0)
        # Windows.Media.Ocr 把每个汉字当一个词；字母数字之间的空格要保留
        self.assertEqual(server.tidy_ocr_text(" 福 利 商 城 "), "福利商城")
        self.assertEqual(server.tidy_ocr_text("扫 10 次"), "扫 10 次")
        self.assertEqual(server.tidy_ocr_text("Lv 20  精 灵 背 包"), "Lv 20 精灵背包")
        self.assertEqual(server.tidy_ocr_text(None), "")

    def test_ensure_ocr_model_honours_manual_path(self):
        sys.path.insert(0, str(SERVER.parent))
        try:
            import egret_agent_inspector_mcp as server
        finally:
            sys.path.pop(0)
        with tempfile.TemporaryDirectory() as tmp:
            model = os.path.join(tmp, "custom.onnx")
            with open(model, "wb") as f:
                f.write(b"not really a model")
            with mock.patch.dict(os.environ, {"EGRET_OCR_MODEL": model}):
                # 手动指定的模型不校验哈希，直接用
                self.assertEqual(server.ensure_ocr_model(), model)
            with mock.patch.dict(os.environ, {"EGRET_OCR_MODEL": os.path.join(tmp, "missing.onnx")}):
                with self.assertRaises(RuntimeError):
                    server.ensure_ocr_model()

    def test_ocr_worker_falls_back_to_native_engine_when_not_ready(self):
        sys.path.insert(0, str(SERVER.parent))
        try:
            import egret_agent_inspector_mcp as server
        finally:
            sys.path.pop(0)
        # worker 没起来时（模型还在下载、Windows ML 不可用）不能报错，要退回平台自带引擎
        self.assertFalse(server._OCR_WORKER_READY.is_set())
        if sys.platform == "win32":
            with mock.patch.object(server.subprocess, "run") as run:
                run.return_value = mock.Mock(returncode=0, stdout='{"matches":[{"id":"7","text":"商 店","confidence":0.7}]}', stderr="")
                png = base64.b64encode(b"\x89PNG\r\n\x1a\n").decode()
                out = server.run_fast_ocr(png, [{"hash": 7, "screenRect": {"x": 1, "y": 2, "width": 30, "height": 20}}], 1)
            self.assertEqual(out["backend"], "windows-media-ocr")
            self.assertEqual(out["texts"], {"7": "商店"})
            self.assertTrue(str(run.call_args[0][0][-2]).endswith("ocr_windows.ps1"))
...
class McpServerTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_reload_names_the_tab_when_several_browsers_are_connected(self):
        # 不点名的重载会落到最后连上来的浏览器（常常是别的会话的 Edge）：扩展只认点名了自己标签页的，
        # 或 server 说只连着这一个浏览器的
        chrome, edge = FakeExtension(), FakeExtension()
        chrome.tabs, edge.tabs = [11], [22]
        await chrome.connect()
        try:
            await self.call("egret_extension_status", {"waitSeconds": 2})
            await self.call("egret_reload_extension", {})
            self.assertEqual(chrome.reloads, [{"sole": True}])
            await edge.connect()
            await self.call("egret_list_tabs", {"probe": False})
            await self.call("egret_reload_extension", {"tabId": 22})
            self.assertEqual(edge.reloads, [{"tabId": 22}])
            self.assertEqual(len(chrome.reloads), 1)
        finally:
            for ext in (chrome, edge):
                ext.task.cancel()
                ext.writer.close()
    ...
    async def test_notes_roundtrip(self):
        ...
        _, route = await self.call("egret_notes", {"action": "add", "scope": "demo", "entries": [{
            "kind": "route", "key": "main-story", "summary": "从地图界面进入主线",
            "start": "地图界面", "steps": [{"tap": {"qaName": "ToolbarNew__btn_task"}}],
            "expect": {"text": "立即前往"}}]})
...
class SceneAndTurnTest(unittest.TestCase):
    """地图界面 HUD 穿透、地图入口借标题、列表项合并，以及回合制界面的等锁 / 连出判定。"""
...
// 地图界面：地图上的飞船热区没有字，「星际探索」四个字摆在热区正下方，自己没有监听
...
(async () => {
    ...
    out.dropAt = dropAt;
    remove(grabItem);
    // 整块面板代收拖动（按 e.target 认拖的是哪一招）：要在手指起点按下，不能按面板中心
    let panelDrag = null;
    const skillPanel = item("petProperty.PetPropertyNor", "otherPanel", splanRoot, { x: 0, y: 0, width: 800, height: 480 });
    const skillRow = item("eui.Group", "skillRow", skillPanel, { x: 600, y: 180, width: 120, height: 70 });
    listenOn(skillPanel);
    skillPanel.addEventListener("touchBegin", function (e) { panelDrag = { from: e.target.name }; }, null);
    skillPanel.addEventListener("touchReleaseOutside", function (e) { panelDrag.to = [e.stageX, e.stageY]; }, null);
    await t.handlers.act({ steps: [{ op: "recommended" }], quietMs: 50, timeoutMs: 200, turnMs: 0 });
    out.panelDrag = panelDrag;
    remove(skillRow);
    remove(skillPanel);
    remove(dropCell);
    // 宽屏：面板居中右移，手的路径还按设计分辨率写死，起点落在已装备的格子上：起点终点一起平移到条目和格子上
    let wideDrag = null;
    const widePanel = item("petProperty.TabViewSkill", null, splanRoot, { x: 0, y: 0, width: 800, height: 480 });
    const wideCell = item("petProperty.SkillCell", null, widePanel, { x: 550, y: 170, width: 120, height: 70 });
    const wideBar = item("petProperty.SkillBar", "SkillBar1learned", widePanel, { x: 700, y: 180, width: 120, height: 70 });
    listenOn(widePanel);
    widePanel.addEventListener("touchBegin", function (e) { wideDrag = { from: e.target.name }; }, null);
    widePanel.addEventListener("touchReleaseOutside", function (e) { wideDrag.to = [e.stageX, e.stageY]; }, null);
    await t.handlers.act({ steps: [{ op: "recommended" }], quietMs: 50, timeoutMs: 200, turnMs: 0 });
    out.wideDrag = wideDrag;
    [wideBar, wideCell, widePanel].forEach(remove);
    ...
    remove(battlePanel);
    remove(splanRoot);
    // 弹窗关完后顶层落在底栏 ToolbarNew 上：已经在地图界面，不去点工具栏上的返回键（点了会离开地图）
    let hudBack = 0;
    hud.visible = false;
    const hubHud = item("ui.ToolbarNew", "ToolbarNew", stage, { x: 0, y: 0, width: 800, height: 480 });
    const hudBackBtn = item("eui.Group", "grp_back_landscape", hubHud, { x: 10, y: 10, width: 60, height: 40 });
    listenOn(hudBackBtn);
    hudBackBtn.addEventListener("touchTap", function () { hudBack++; }, null);
    const hubRun = (await t.handlers.act({ steps: [{ op: "close" }], quietMs: 50, timeoutMs: 200, turnMs: 0 })).executed[0];
    out.hubClose = { stopped: hubRun.result && hubRun.result.stopped, back: hudBack };
    out.hubPanel = hubRun.result && hubRun.result.panel;
    // 登录后回到地图界面时接连弹一串礼包：刚好用完循环次数时最后一层已关掉，也要认出回到了地图界面
    for (let k = 0; k < 2; k++) {
        const pop = item("gift.GiftPop", "giftPop" + k, stage, { x: 0, y: 0, width: 800, height: 480 });
        item("eui.Image", "giftBg" + k, pop, { x: 0, y: 0, width: 800, height: 480 });
        const popClose = item("eui.Button", "_btnClose", pop, { x: 740, y: 10, width: 50, height: 50 });
        listenOn(popClose);
        popClose.addEventListener("touchTap", function () { remove(pop); }, null);
    }
    const allRun = (await t.handlers.act({ steps: [{ op: "close", all: true, max: 2 }], quietMs: 50, timeoutMs: 200, turnMs: 0 })).executed[0].result;
    out.closeAllMany = { ok: allRun.ok, stopped: allRun.stopped, layers: allRun.layers.length };
    remove(hudBackBtn);
    remove(hubHud);
    hud.visible = true;
    delete window.MFC;
...
'''
    ...
    def test_card_that_only_works_when_dragged_out_is_swiped(self):
        ...
        # 按下时也挂松手回调、但不看方向或四边都判的普通按钮不算拖动（地图界面一排按钮曾被误标）
    ...
    def test_splan_drag_guide_is_dragged_along_the_hand_path(self):
        ...
        self.assertEqual(data["panelDrag"], {"from": "skillRow", "to": [510, 205]})
        self.assertEqual(data["wideDrag"], {"from": "SkillBar1learned", "to": [610, 205]})
    ...
    def test_splan_lost_session_and_fixed_labels(self):
        ...
        self.assertEqual(data["hubClose"], {"stopped": "hub", "back": 0}, data.get("hubPanel"))
        self.assertEqual(data["closeAllMany"], {"ok": True, "stopped": "hub", "layers": 2})
...
class RenderTableTest(unittest.TestCase):
    ...
    def test_newbie_progress_and_guide_run_are_spelled_out(self):
        ...
        self.assertIn('一次关回地图界面：[{"op":"close","all":true}]', text)
        closed = server.render_action_table({"marker": "g6", "actions": [], "executed": [
            {"op": "close", "result": {"all": True, "ok": True, "stopped": "hub",
                                       "layers": ["payPlusPop.PayPlusPop#payPlusPop.PayPlusPop", "ApplicationViewAdvanced#rayGrowthPlan.RayGrowthPlan"]}},
            {"op": "close", "result": {"all": True, "ok": False, "stopped": "stuck", "layers": [],
                                       "panel": "bookPanel.BookPanel", "note": "都点过了"}}]})
        self.assertIn("执行 close → 关掉 2 层（PayPlusPop、RayGrowthPlan），已回到地图界面", closed)
        self.assertIn("执行 close → 关掉 0 层，停在 bookPanel.BookPanel（stuck）：都点过了", closed)
    ...
    def test_route_ends_where_it_returns_to_the_main_scene(self):
        ...
        for step in [("sign_close", sign, gift), ("gift_close", gift, ret), ("ret_close", ret, main, True),
            ...
            tap(*step)
        # 这个任务登录后是同一串弹窗：路线到回地图界面为止，不带上个任务的 PVE
